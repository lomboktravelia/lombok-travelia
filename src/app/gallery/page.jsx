    ...
    <div>
      <section className="text-center py-20 bg-gray-100">
        <div className="flex flex-col mx-auto justify-center items-center text-black">
          <h2 className="text-4xl font-bold mb-8 text-green-600">GALERI</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-5 w-full max-w-[1450px]">
            {pictures.map((picture) => (
              <div key={picture.id_gallery} className='h-[300px] flex justify-center items-center rounded-lg shadow-md overflow-hidden'>
                <Image src={picture.image_url} alt={`Foto Wisata ${picture.id_gallery}`} width={400} height={300} className="rounded-lg object-cover transition-transform duration-300 transform hover:scale-105" />
              </div>
            ))}
          </div>
